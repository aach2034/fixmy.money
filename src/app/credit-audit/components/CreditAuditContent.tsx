...
interface AuditResult {
  ...
  priorityItems: { creditor: string; type: string; bureau: string; amount: string; reason: string; strength: string; anomaly: string }[];
...
export default function CreditAuditContent() {
  ...
  const runAudit = async (client: AuditClient) => {
    ...
    try {
      ...
      const strongCount = negativeItems.filter(item => item.disputeStrength.strengthLabel === 'Strong').length;
      const moderateCount = negativeItems.filter(item => item.disputeStrength.strengthLabel === 'Moderate').length;
                      ...
                      <div key={i} className="flex items-start gap-3 p-3 bg-muted/30 rounded-lg">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-2">
                            <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">Dispute Strength: {item.strength}</span>
                            <span className="text-xs text-muted-foreground">{item.anomaly}</span>
                          </div>
