          ...
          {!auditResult ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {[
                  { label: 'Negative accounts', value: auditResult.negativeItemCount, color: 'text-slate-950' },
                  { label: 'Items worth reviewing', value: auditResult.priorityItems.length, color: 'text-slate-950' },
                  { label: 'Strong opportunities', value: auditResult.priorityItems.filter(item => item.strength.toLowerCase().includes('strong')).length, color: 'text-green-700' },
                ].map(s => (
              ...
              <div className="flex gap-2">
                ...
                <a href={`/dispute-wizard?clientId=${auditResult.clientId}&clientName=${encodeURIComponent(auditResult.clientName)}&fromReport=true${requestedReportId ? `&reportId=${encodeURIComponent(requestedReportId)}` : ''}`} onClick={() => trackOrganicConversionStep('dispute_wizard_start_clicked', { source: 'credit_audit' })} className="btn-primary flex items-center gap-1.5 text-sm">
