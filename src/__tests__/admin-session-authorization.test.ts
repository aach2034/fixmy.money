...
describe('database-authoritative administrator session authorization', () => {
  it('accepts Adam-style active superadministrator state through authenticated RLS', async () => {
    ...
    await expect(
      getAuthenticatedPlatformAdminRole(query.client, 'adam-auth-user-id')
    ...
    expect(query.userEquals).toHaveBeenCalledWith('user_id', 'adam-auth-user-id');
  ...
  it.each([
    ...
    await expect(
      getAuthenticatedPlatformAdminRole(query.client, 'unprivileged-user-id')
    ).resolves.toBeNull();
  });
});
